...
class CastrelSignIntegrationControllerTest {
  ...
  void adminDownloadsHostnameAutoEnrollmentPackageZipWithoutClientMaterial() throws Exception {
    ...
    org.assertj.core.api.Assertions.assertThat(entries.get("agent.yaml"))
        ...
        .contains("batch_interval: 30s")
        .contains("sender_flush_interval: 2s")
        .contains("max_spool_record_bytes: 8mb")
        .contains("max_spool_bytes: 256mb")
        .contains("max_spool_records: 10000")
        .contains("max_spool_age: 168h")
        .contains("max_batch_items: 1000")
        .contains("max_batch_bytes: 4mb")
        .contains("max_item_bytes: 512kb")
        .contains("log_message_max_bytes: 1024")
        .contains("collector_interval_identity: 1h")
        .contains("collector_interval_metric: 30s")
        .contains("collector_interval_network: 5m")
        .contains("collector_interval_process: 2m")
        .contains("collector_interval_service: 5m")
        .contains("collector_interval_port: 2m")
        .contains("collector_interval_package: 12h")
        .contains("collector_interval_firewall: 10m")
        .contains("collector_interval_log_tailer: 30s")
        .contains("collector_interval_agent_health: 30s")
        .contains("collector_full_interval_identity: 24h")
        .contains("collector_full_interval_network: 1h")
        .contains("collector_full_interval_process: 15m")
        .contains("collector_full_interval_service: 1h")
        .contains("collector_full_interval_port: 15m")
        .contains("collector_full_interval_package: 24h")
        .contains("collector_full_interval_firewall: 1h")
        .contains("update_enabled: true")
        .contains("update_channel: stable")
        .contains("update_public_key_path: ./update_public_key.pem")
        .contains("file_manager_enabled: false")
        .contains("file_manager_allow_delete: false")
        .contains("file_manager_poll_interval: 30s")
        .contains("file_manager_roots:")
        .contains("  - __FILE_MANAGER_ROOT__")
        .contains("  - identity")
        .contains("  - metric")
        .contains("  - network")
        .contains("  - process")
        ...
        .contains("  - firewall")
        .contains("  - log_tailer")
        .contains("  - agent_health")
        .doesNotContain("ingest_transport: https")
        .doesNotContain("  - /")
    ...
    org.assertj.core.api.Assertions.assertThat(entries.get("install.ps1"))
        ...
        .contains("__FILE_MANAGER_ROOT__")
        .contains("$fileManagerDir = Join-Path $installRoot 'files'")
        .contains(".Replace('__FILE_MANAGER_ROOT__', $fileManagerDir)")
        .contains("$utf8NoBom = New-Object System.Text.UTF8Encoding($false)")
        .contains("[System.IO.File]::WriteAllText($configPath, $renderedConfig, $utf8NoBom)")
        .doesNotContain("Set-Content -Path $configPath -Encoding utf8")
        .contains("$env:ProgramData")
        .contains("$updateDir = Join-Path $installRoot 'update'")
        .contains("CastrelyxAgent")
        .contains("New-Service")
        .contains("sc.exe failure $serviceName")
        .contains("restart/5000/restart/5000/restart/30000")
    ...
    org.assertj.core.api.Assertions.assertThat(entries.get("install.sh"))
        ...
        .contains("__HOSTNAME__")
        .contains("__FILE_MANAGER_ROOT__")
        .contains("file_manager_dir=\"$state_dir/files\"")
        .contains("s|__FILE_MANAGER_ROOT__|/var/lib/castrelyx-agent/files|g")
        .contains("/etc/castrelyx/agent.yaml")
        .contains("/etc/systemd/system/castrelyx-agent.service")
        .contains("install -d -o root -g root -m 0700 \"$state_dir\" \"$cert_dir\" \"$spool_dir\" \"$update_dir\" \"$file_manager_dir\"")
        .contains("chown root:root \"$config_path\" \"$cert_dir/ca.pem\" \"$config_dir/update_public_key.pem\"")
        .contains("chown root:root \"$service_path\"")
        .contains("chmod 0644 \"$service_path\"")
        .contains("User=root")
        .contains("Group=root")
        .contains("NoNewPrivileges=true")
        .contains("ProtectSystem=strict")
        .contains("ProtectHome=read-only")
        .contains("RestrictAddressFamilies=AF_UNIX AF_INET AF_INET6 AF_NETLINK")
        .contains("ReadOnlyPaths=/etc/castrelyx /var/log")
        .contains("ReadWritePaths=/var/lib/castrelyx-agent /usr/local/bin")
        .contains("LimitNOFILE=65536")
        .contains("TasksMax=512")
        ...
        .contains("update_public_key.pem")
        .doesNotContain("ProtectProc=", "ProcSubset=", "PrivateDevices=true");
