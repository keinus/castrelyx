...
func Load(path string) (Config, error) {
  ...
  cfg := Config{
    ...
    EnabledCollectors: []string{"identity", "metric", "network"},
  }
  ...
  for scanner.Scan() {
    ...
    if inCollectors && strings.HasPrefix(line, "-") {
      ...
      if value != "" {
        if len(cfg.EnabledCollectors) == 3 && cfg.EnabledCollectors[0] == "identity" && cfg.EnabledCollectors[1] == "metric" && cfg.EnabledCollectors[2] == "network" {
